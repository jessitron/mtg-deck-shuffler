...
describe("formatGamePageHtmlPage playmat rendering", () => {
  test("with a picked playmat: renders it as the playmat's background-image", () => {
    ...
    const game = GameState.newGame(1, 1, 1, deck, undefined, undefined, undefined, "/images/playmats/aeoe-3-exalted-sunborn.png");
    game.startGame();

    const html = formatGamePageHtmlPage(game);
    expect(html).toContain(`background-image: url('/images/playmats/aeoe-3-exalted-sunborn.png')`);
  });
