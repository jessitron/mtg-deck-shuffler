...
describe("table look on GameState", () => {
  test("newGame with a sleeve/playmat carries it into the persisted state and back", async () => {
    ...
    const game = GameState.newGame(1, 1, 1, deck, undefined, undefined, "#bd0a0a", "/images/playmats/aeoe-43-cascading-cataracts.png");

    const persisted = game.toPersistedGameState();
    expect(persisted.sleeveColor).toBe("#bd0a0a");
    expect(persisted.playmatImagePath).toBe("/images/playmats/aeoe-43-cascading-cataracts.png");
    ...
    expect(reloaded.playmatImagePath).toBe("/images/playmats/aeoe-43-cascading-cataracts.png");
  });
