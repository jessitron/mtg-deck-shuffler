...
- **`services/spine/app.rb`'s `current_span`/`mark_span_failed` helpers** (ticket 03,
  `spine-roda-rewrite`, 2026-08-11): the Spine's first hand-rolled span-attribute code —
  `current_span.add_attributes(...)` for inputs/outcome on `POST /join`, `mark_span_failed(result,
  error)` for the three failure branches (`invalid_input`/`seat_occupied`/`table_full`), setting
  `join.result` plus `current_span.status = OpenTelemetry::Trace::Status.error(...)`. Both live
  directly in `app.rb`, not a `tracing_util.rb` — there's only one route so far. Copy this shape for
  the next route rather than reinventing attribute names; extract to a shared file once a second
  route needs the same helpers. **`join.result` does not currently distinguish `created` from
  `joined`** — both a brand-new table and joining an existing one set `join.result => "joined"`; if a
  future ticket wants that distinction, it needs a new value, not an assumption that `"created"`
  already exists. Unrelated to the `traceparent`-in-body gap above (different helper, different
  target — live span vs. persisted event).
