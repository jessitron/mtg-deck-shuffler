...
- **The Spine's `/admin/tables`** — renders per-event Honeycomb trace links; assumes trace context propagates Shuffler → Spine (Rack extracts W3C headers). Confirmed live end-to-end by `shuffler-posts-to-spine` (2026-08-10): the Shuffler's `card.played` send is the first real HTTP traffic on this edge — previously the assumption was untested.
