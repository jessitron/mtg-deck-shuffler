...
- **Adding a new telemetry init path, or a fourth ship**: once `build-sha-on-every-span` (`TODO.md`) lands, it must carry the deployed version as a resource attribute (README → Invariant 5). Until then, note that no span identifies its build.
