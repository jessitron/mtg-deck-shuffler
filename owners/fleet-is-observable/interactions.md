...
- **The `traceparent`-in-body minting sites** — same `00-{traceId}-{spanId}-{flags}` format, each reading its own tracing API's ambient span. **Down to two live sites since the Rails Spine was deleted whole** (ticket 01 of `spine-roda-rewrite`, 2026-08-10): `apps/tabletop/src/client/observability/index.ts` and `apps/shuffler/src/port-tabletop/traceparent.ts`. `services/spine/app/controllers/application_controller.rb`'s `current_traceparent` is gone with the rest of the app; the Roda rewrite will need an equivalent never-`nil` minting site (README → "Trace context embedded in event bodies" has the shape to copy) before it can round-trip a `traceparent` into its own persisted events again. This is separate from HTTP-header propagation (automatic, via undici auto-instrumentation); it exists because `contracts/envelope.v1.json` requires a durable `traceparent` value in the persisted event body, which a header can't provide once the request is over. **Still exactly three sites, not four**: the Shuffler's new Spine send (`shuffler-posts-to-spine`, 2026-08-10, `src/port-spine/sendToSpine.ts`) reuses the Shuffler's existing site (`buildCardPlayedEvent` → `currentTraceparent()`) rather than minting its own — it just calls that same function a second time per play (once for the Tabletop's envelope, once for the Spine's), so the two envelopes get different event `id`s but an identical `traceparent` value, since both calls read the same still-active request span.

## Depended on by

- **Every production diagnosis** — the North Star includes "when something breaks, Honeycomb shows you why."
- **The Spine's `/admin/tables`** — historical: it rendered per-event Honeycomb trace links and assumed trace context propagates Shuffler → Spine (Rack extracts W3C headers), confirmed live end-to-end by `shuffler-posts-to-spine` (2026-08-10). The whole Rails Spine, `/admin/tables` included, was deleted the same day (ticket 01 of `spine-roda-rewrite`); the Roda rewrite will need an equivalent admin view before this dependency is real again.
...
- **A new service/ship**: OTel from its first commit (`notes/add-opentelemetry.md` is the runbook). **This now includes the incoming Roda Spine** — the Rails Spine's OTel init, sampler, and traceparent-minting site were all deleted with the rest of the app (ticket 01, 2026-08-10); tickets 02-06 of `spine-roda-rewrite` build a new service that needs this runbook followed from its first commit, with no sampler port-over (the spec explicitly excludes `BackgroundChatterSampler` — flagged broken — and starts at 100% sampling).
