...
- **Adding a span type to a high-volume emitter**: threshold it and roll the small ones into attributes on the span that already exists (`test.expect.count`/`.total_ms`/`.suppressed_count` is the worked example). Invariant 1 is also a volume control.
- **Deciding whether a trace is "too big"**: don't re-guess it — **~10,000 spans is where a waterfall gets hard to read; ~1,000 is comfortable** (Jess, 2026-08-07, from the real 1,090-span harness waterfall; three agent guesses before that were all too conservative). That is a *usability* ceiling only. **Spans/run against env `local`'s budget is a separate question with a separate answer** — a dev tool must not become the environment's largest span source. README → Dev-tooling telemetry.
