...
- **The `traceparent`-in-body minting sites — resolved for the Spine's inbound path, ticket 04, 2026-08-11, but NOT by adding a minting site.** Same `00-{traceId}-{spanId}-{flags}` format, each reading its own tracing API's ambient span. `services/spine/app/controllers/application_controller.rb`'s `current_traceparent` was deleted with the rest of the Rails app (ticket 01, 2026-08-10); ticket 03 landed `POST /join`'s `mint_event!` with no `traceparent` field, leaving a live (not hypothetical) gap. **Ticket 04 closed that gap by changing the contract instead of adding the helper**: `contracts/envelope.v3.json` drops `traceparent` as an envelope field entirely — `additionalProperties: false`, no property named `traceparent`. Inbound (`POST /tables/:table_id/events`), trace context travels only via the HTTP header, extracted automatically by Rack instrumentation; the persisted `Event` row gets no trace column, and now never will for inbound events, by design (`services/spine/CLAUDE.md`'s Observability section, and the accepted tradeoff that pre-SSE events have no durable trace link). Outbound, ticket 05's SSE stream is specified to carry `meta.traceparent` as a **sibling** field alongside the envelope, not merged into it — not yet implemented. **Still two live minting sites, but the roster changed**: `apps/tabletop/src/client/observability/index.ts` (Tabletop, optional/websocket, unaffected) and `apps/shuffler/src/port-tabletop/traceparent.ts` (Shuffler, required, unaffected for its original Tabletop-facing use) — but the Shuffler's `HttpSpineGateway.sendEvent` (`shuffler-posts-to-spine`, 2026-08-10) reuses that same helper to build the envelope it sends **to the Spine**, and that reuse is now wrong: `envelope.v3.json`'s `additionalProperties: false` rejects the `traceparent` field that helper always includes. Filed as the `shuffler-spine-gateway-stale` buoy in `TODO.md` (2026-08-11) rather than fixed in this ticket — the gateway also still calls routes ticket 03 deleted (`/tables/lookup`, `/tables`, `/tables/:id/seats` instead of `/join`), and nothing in production calls it successfully today. README → "Trace context embedded in event bodies" has the full rewrite; the deleted Rails `current_traceparent`'s `SecureRandom.hex`-fallback shape is kept there only as a historical reference now, not a pattern to copy for the Spine's inbound path.
...
- **`services/spine/app.rb`'s `current_span`/`mark_span_failed` helpers** (ticket 03 landed them,
  ticket 04 generalized `mark_span_failed`, `spine-roda-rewrite`, 2026-08-11): the Spine's
  hand-rolled span-attribute code — `current_span.add_attributes(...)` for inputs/outcome on both
  `POST /join` and `POST /tables/:table_id/events`, and now one shared `mark_span_failed(attribute,
  result, error)` — the attribute key (`"join.result"` or `"event.result"`) is passed by the
  caller rather than baked into the helper, since a second route needed a different attribute name
  for the same shape. Both still live directly in `app.rb`, not a `tracing_util.rb` — two routes
  share them now; extract to a shared file if a third route needs them too. **`join.result` does
  not currently distinguish `created` from `joined`** — both a brand-new table and joining an
  existing one set `join.result => "joined"`; if a future ticket wants that distinction, it needs a
  new value, not an assumption that `"created"` already exists. **`event.result`'s dedup path does
  get its own value** (`"duplicate"`, distinct from `"accepted"`) — confirmed on review, don't
  collapse it back into `"accepted"` if `mark_span_failed`/success-path code here is touched again.
  Unrelated to the `traceparent`-in-body question below (different helper, different target — live
  span vs. persisted event) — though ticket 04 resolved that question for the Spine's inbound path
  by changing the contract, not by adding a sibling helper here.
...
- **Adding or editing a `traceparent`-in-body minting helper** (Tabletop `currentTraceparent`, Shuffler `traceparent.ts`): keep the `00-{traceId}-{spanId}-{flags}` format identical across both. If the field the caller writes into is required by a JSON Schema contract, the helper must never return `undefined`/`nil` — synthesize a well-formed random one, and flag the synthesis on the active span (`traceparent.synthesized: true`, the Shuffler's addition) so a real occurrence in production is visible rather than a silent trace-shaped string that links nowhere. If the field is optional and only used for point-to-point propagation (like the Tabletop's websocket URL), `undefined`-on-no-span is correct and no flag is needed. **Do not add a Roda-Spine equivalent for inbound events** — `envelope.v3.json` (ticket 04) deliberately has no `traceparent` property for the Spine to receive; that gap is closed by the contract, not a future helper. README → "Trace context embedded in event bodies". **Do check `apps/shuffler/src/port-tabletop/traceparent.ts`'s existing call sites before assuming they're all still valid** — `HttpSpineGateway.sendEvent` calls it to build an envelope that `envelope.v3.json` now rejects (`additionalProperties: false`); see the `traceparent`-in-body bullet above and the `shuffler-spine-gateway-stale` buoy.
