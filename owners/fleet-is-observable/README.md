...
`addEventListener`. Keep it that way. **`services/spine` has real Ruby app code again as of
ticket 02 of `spine-roda-rewrite` (2026-08-11): `app.rb`, `config/telemetry.rb` — neither calls
`addEvent`/`add_event`, confirmed by reading both files. The grep above should now cover
`services/spine` for real** (it previously only had the Node ships to check, since ticket 01
deleted the whole Rails app).
...
**Both Node ships are now on the same OTel version line — 0.221 (since `align-otel-versions`,
2026-08-10).** They used to diverge (Shuffler 0.219, Tabletop 0.221), and the same class can have a
different constructor across that split: `new BatchLogRecordProcessor(exporter)` in 0.219 vs
...
files therefore get a test in *each* ship — that is the only reason this was caught: bumping the
Shuffler's `api-logs`/`auto-instrumentations-node`/`exporter-logs-otlp-http`/`exporter-trace-otlp-http`/`sdk-logs`/`sdk-node`
from 0.219→0.221 (`core`/`resources`/`sdk-trace-base` 2.8.0→2.10.0, `instrumentation-express`
0.67.0→0.69.0 to match what `auto-instrumentations-node@0.79.0` bundles) turned
`apps/shuffler/test/log.test.ts`'s `SimpleLogRecordProcessor(exported)` into a real typecheck
failure, fixed to the options-object shape (`{ exporter: exported }`) alongside the same fix in
`apps/shuffler/src/tracing.ts`.

**This convergence is a point-in-time fact, not a standing guarantee.** Nothing pins the two ships'
`package.json`s to the same version — the next time either ship's OTel deps get bumped alone, the
split (and the constructor-shape landmine above) can reopen silently. Whoever bumps one ship's OTel
deps should check the other's before assuming the shapes still match.
...
| `apps/tabletop/src/server/tracing.ts`                 | A **separate** Node SDK init, "modeled on the Shuffler's". Own inline `KubeProbeAwareSampler` (0.001 kube-probe / 0.01 ELB). **No middleware suppression, no static-asset or `/health` handling, reads only `http.user_agent`, and no test.** See Watch points. Same `logRecordProcessors` wiring as the Shuffler — since `align-otel-versions` (2026-08-10) both ships are on the 0.221 options-object constructor, but the code comment here now frames the 0.219/0.221 split as history to recheck on the next version bump, not a live fact. **Since `19e1bdf` (2026-08-10, `tabletop-no-shutdown-flush`)** also calls `installShutdownHandlers(() => sdk.shutdown(), { onTimeout, onDrainError })` right after `sdk.start()`, same as the Shuffler — see the `shutdownHooks.ts` row below.                                                                                                                                                                                       |
...
- **Ticket 03 of `spine-roda-rewrite` (2026-08-11) — the Spine's first hand-rolled span attributes,
  ...
  `player.name`) and outcome (`join.result`, `seat.number`), and a new `mark_span_failed(result,
  error)` helper that sets `join.result` plus `current_span.status =
  OpenTelemetry::Trace::Status.error(...)` on every failure branch
  (`invalid_input`/`seat_occupied`/`table_full`). Both helpers live directly in `app.rb` — no
  `tracing_util.rb` yet, since there's only the one route to serve. **One gap between the plan and
  the code, worth watching**: the route only ever sets `join.result => "joined"` on success — it
  does not distinguish a brand-new table (`Table.create_with_event!`) from joining an existing one,
  so a `created` value some prior framing expected does not actually appear. Confirm before relying
  on `join.result == "created"` in a query; today it's `joined` either way. The
  `traceparent`-in-event-body gap this KB already tracked (`models/table.rb`'s `mint_event!` has no
  `traceparent` field) is untouched by this ticket — these span attributes stamp the live request
  span, not the persisted `Event` row — and stays deferred to ticket 04/05.
...
- `align-otel-versions` (2026-08-10) — the Shuffler's OTel deps were bumped from the 0.219 line to
  ...
  `sdk-trace-base` 2.8.0→2.10.0; `instrumentation-express` 0.67.0→0.69.0, the version
  `auto-instrumentations-node@0.79.0` itself bundles). This closes the version-line divergence this
  file used to describe as current state. **Good evidence the constructor-shape landmine is real,
  not theoretical**: `apps/shuffler/test/log.test.ts`'s `SimpleLogRecordProcessor(exported)` (0.219
  positional shape) failed to typecheck the moment the dependency moved, fixed to
  `SimpleLogRecordProcessor({ exporter: exported })` alongside the same fix already needed in
  `apps/shuffler/src/tracing.ts`'s `BatchLogRecordProcessor`. Verified beyond `npm run build`/`npm
  test` (353 Shuffler tests, 97 Tabletop tests green): started the built Shuffler against a
  throwaway local OTLP HTTP catcher, curled `/`, and confirmed the captured payload still has
  exactly 2 spans (root `GET /` + `request handler - /`, both with `http.route: "/"`), with
  `instrumentation-express`/`instrumentation-http` scope versions reading 0.69.0/0.221.0 — the ESM
  loader patching and the `ignoreLayersType: [MIDDLEWARE]` suppression both survived the bump. **Not
  a standing guarantee**: nothing pins the two ships to the same version going forward; the split
  can reopen the next time either ship's OTel deps are bumped alone. No shared telemetry package
  was extracted (still a deliberate per-ship duplication) and the samplers were left alone
  (`BackgroundChatterSampler` vs `KubeProbeAwareSampler` still diverge — orthogonal, not touched).
- `shuffler-posts-to-spine` (2026-08-10) — the Shuffler started sending `card.played` to the
  Spine's event log over HTTP (`src/port-spine/`), alongside its existing send to the Tabletop.
  Notable *for this owner* only because of what it didn't add: no new `traceparent`-in-body
  minting site (reused `port-tabletop/traceparent.ts`'s `currentTraceparent()` as-is), no manual
  spans (relied on undici auto-instrumentation for the outbound fetch, same as every other
  server-to-server call on this fleet), and the same best-effort failure shape as
  `sendSeatJoinedBestEffort` (span attribute + `log.warn`, never throws). Confirms live, for the
  first time, that a real Shuffler → Spine HTTP call actually produces a `card.played` event the
  Spine's contract validation accepts and `/admin/tables` can link back to a trace — previously
  that path was only asserted, not exercised end-to-end. See README → wiring table (new
  `port-spine/` row) and `owners/fleet-is-observable/interactions.md`.
...
- **2026-08-10: `envelope.v2.json` added `origin` and `significance`; both landed on the span too.**
  `Table#record_span_attributes` (`services/spine/app/models/table.rb`) now stamps `event.origin`
...
- **2026-08-11, ticket 02 of `spine-roda-rewrite`: the Spine got OTel wiring back, from its first
  boot-only commit.** `config/telemetry.rb` + `app.rb` + `run` + `.env`. Two findings worth
  keeping:
  - **Roda/Rack has no auto-injection point for an instrumentation gem, and the failure mode is
    total silence.** Rails' railtie hooks a gem in automatically; `opentelemetry-instrumentation-rack`
    only *registers* itself via `SDK.configure`'s `use` — it produces **zero spans** until the app
    class body explicitly does `use(*OpenTelemetry::Instrumentation::Rack::Instrumentation.instance.middleware_args)`.
    Without that line: app boots clean, logs "Rack was successfully installed", every request
    returns 200 — and nothing reaches Honeycomb, with no error anywhere to notice. Confirmed by
    reading the installed gem's `instrumentation.rb`: `middleware_args` exists specifically to hand
    back the right `TracerMiddleware` variant (Stable/Old/Dup, keyed on `OTEL_SEMCONV_STABILITY_OPT_IN`).
    This is the seventh instance of this file's fails-open-invisibly pattern, and the first one
    that's Rack-specific — worth checking on the next Rack-based instrumentation gem regardless of
    which ship adds it.
  - **Querying the wrong MCP server looks identical to a wiring bug.** The default `honeycomb` MCP
    server points at an unrelated demo team with no fleet data; this fleet's data is under
    `honeycomb-modernity` (team `modernity`, matching root `CLAUDE.md` → Observability). A "no
    spans found" query against the wrong server cost a false alarm before the right server
    confirmed the `GET` root span was there all along, dataset `spine`, env `local`.
  - Deliberately **not** ported: `BackgroundChatterSampler` — the rewrite spec flags it broken and
    starts this service at 100% sampling instead, matching the "new service/ship" watch point in
    `interactions.md`. `services/spine/CLAUDE.md` now documents both findings for the next person
