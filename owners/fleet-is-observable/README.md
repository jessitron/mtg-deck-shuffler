...
`"undici": "^7.29.0"` for exactly this reason, and **the Shuffler's `package.json` now pins the
identical `"undici": "^7.29.0"`** — its first `undici` dependency, added for its own Spine SSE
subscriber (`apps/shuffler/src/port-spine/spineSubscriber.ts`, confirmed no drift against the
Tabletop's pin before landing). `Agent`/`Dispatcher` are not Node builtins — there
...
| Where | What it is |
| --- | --- |
| `apps/shuffler/src/tracing.ts` | Node SDK init. ESM loader hook (`register("@opentelemetry/instrumentation/hook.mjs")`) + `node --import`. Auto-instrumentations; `fs` off; **Express middleware spans off** (`ignoreLayersType: [ExpressLayerType.MIDDLEWARE]`) — keeps a typical trace at 2 spans instead of 8. **`@opentelemetry/instrumentation-http`'s `ignoreIncomingRequestHook` now also suppresses the incoming-request span for `GET /game-events/:gameId`** (`ignoreIncomingRequestHook: (req) => isGameEventsStreamPath(req.url)`, matched via the pure predicate in `apps/shuffler/src/game-events-route-filter.ts`) — the fleet's first use of this instrumentation-config hook, and now the reference shape for suppressing the span on any future long-lived/tab-lifetime inbound connection route (see the wiring-table row and watch point below). `ParentBasedSampler({root: BackgroundChatterSampler})`. `logRecordProcessors: [BatchLogRecordProcessor(OTLPLogExporter)]` — logs ride the same NodeSDK so they share the resource (`service.name`) and the shutdown-flush path with traces. **Passing `logRecordProcessors` makes the SDK skip its `OTEL_LOGS_EXPORTER` branch entirely**, so that env var is dead config on these two ships — don't add it. (The Spine's `OTEL_LOGS_EXPORTER=none` is real; it has no pipeline.) |
...
| `apps/tabletop/src/server/sendCardReturned.ts` + `apps/tabletop/src/server/cardReturned.ts` | **The Tabletop's own best-effort outbound send** (library-portal ticket 12) — a different direction from `apps/shuffler/src/port-spine/`'s `HttpSpineGateway` (Shuffler→Spine for `card.played`): here it's Tabletop-server→Spine, `POST /tables/:tableId/events` with a `card.returned.v1` payload. `sendCardReturnedToSpineBestEffort` never throws (same house style as the Shuffler's best-effort sends), but unlike them it **resolves `Promise<boolean>`** (`true` on a 2xx, `false` on any failure) rather than being pure fire-and-forget — its caller, `handleCardReturned` (the new `POST /api/tables/:tableName/cards/return` route, the first client-to-Tabletop's-own-server POST from production client code), is a genuine send-then-commit gate: the browser only deletes the swallowed card's shape after a 200, and gets a 502 to keep it on a failure. Both outcomes are stamped on the active span — `card.returned.spine_confirmed: true`/`false` — and failure additionally sets `spine_send.send_failed: true` (kept for continuity with the Shuffler's existing `<name>.send_failed` spelling) plus a `log.warn`. Bounded by a single-request `AbortSignal.timeout(5000)`, not the long-lived `undici.Agent`/`createHeartbeatAwareDispatcher` shape `spineSubscriber.ts` uses — this is one request, not an idle stream. `baseUrl` reads `process.env.SPINE_URL` **at call time** (a default parameter, not a frozen module-level const), so it's testable and a runtime env change takes effect without a restart-order dependency. **Not yet implemented**: a bounded-vocabulary `spine_send.failure_reason` (`timeout`/`non_2xx`/`network_error`) attribute distinguishing *why* the send failed — flagged as a reasonable fast-follow, left out of ticket 12's scope. |
| `apps/tabletop/src/server/testSeedRoute.ts` | **Test-only HTTP seam.** `POST /test/tables/:tableName/cards` calls the same `applyCardArrival` the SSE dispatcher uses, but is mounted **only when `ENABLE_TEST_SEED_ROUTE=true`** (set by `apps/tabletop/verify.sh` and by `cardArrival.test.ts`/`furnitureZOrder.test.ts`, which each spawn a real server process and need a way to seed a card with no live Spine to publish through). Carries no manual span or extra attributes of its own — same ambient-request-span coverage as any other Express route, nothing telemetry-specific added. **Never mounted without the env var, never in production** — if you're looking for how `card.played` reaches the canvas in prod, it's the SSE dispatcher above, not this. |
| `apps/shuffler/src/port-spine/spineSubscriber.ts` + `gameSubscriptionRegistry.ts` + `cardReturnedDispatch.ts` | The Shuffler's own live Spine SSE subscription (seat-session-attribution/table-mode ticket 03) — the reverse leg of the Tabletop's row above, and this ship's first inbound-events consumer. `spineSubscriber.ts` is a straight port of the Tabletop's file of the same name: hand-rolled SSE client, same heartbeat-aware bounded `headersTimeout`/`bodyTimeout` undici `Agent` dispatcher (`HEADERS_TIMEOUT_MS = 5_000`, `BODY_TIMEOUT_MS = 45_000`), no ambient span — reconnect/parse-failure conditions go through `log.warn` only, same as the Tabletop's copy. `gameSubscriptionRegistry.ts` is an in-memory `Map<gameId, entry>` opened idempotently by `GET /game-section/:gameId` (`app.ts`) whenever `persistedGame.spineTableId` is set and no live entry exists yet — covers first load, HTMX re-fetch, and "came back after a while" with one check, no telemetry of its own. `cardReturnedDispatch.ts` (**the Shuffler's first manual spans** — see the "Manual span creation" note below) dispatches `card.returned` events; every SSE event kind still gets the receiving span (`` `sse subscription: ${event.name}` ``), matching the fleet's SSE-event-standard convention the Tabletop's `spineEventDispatch.ts` established. `incomingEventValidation.ts` is this ship's first *inbound* contract gate — ajv-validates what arrives, mirroring the Tabletop's `contractValidation.ts`, distinct from the pre-existing `test/port-spine/contractValidation.ts` which only validates what this ship *sends*. **New production runtime dependency on `contracts/`**: the Shuffler's `Dockerfile` flattens the workspace to `/app` (unlike the Tabletop's, which preserves `/repo/apps/tabletop` nesting), so `incomingEventValidation.ts`'s default `process.cwd()`-relative path to `contracts/` can't resolve in the built image — fixed with a `CONTRACTS_DIR` env var override (mirroring the Spine's own `CONTRACTS_DIR`, `services/spine/lib/event_contract.rb`), the Dockerfile now `COPY contracts ./contracts` and `ENV CONTRACTS_DIR=/app/contracts`. Deliberately **no `import.meta.url`** in this file (unlike the Tabletop's `contractValidation.ts`) — ts-jest's ESM transform errors on it and this file is reached transitively by the new jest test, so it uses the same `process.cwd()`-relative workaround already used in `test/port-spine/contractValidation.ts`. Teardown (closing the subscription when the last browser tab disconnects) is still out of scope; this registry entry, once opened, stays open. **The browser-facing `GET /game-events/:gameId` push landed in ticket 04** — see the dedicated row below. |
| `apps/shuffler/src/app.ts` `GET /game-events/:gameId` + `apps/shuffler/src/game-events-route-filter.ts` | The browser-facing leg of the Shuffler's Spine subscription (ticket 04) — a native browser `EventSource`, one connection per open tab, fed by `addBrowserStream(gameId, stream)`. **It deliberately holds no lifetime-spanning span**: the connection lives as long as the browser tab does, so an incoming-request span covering it would be meaningless (no natural end, would dwarf every other span in any trace it touched). The route's span is suppressed via the `ignoreIncomingRequestHook` wired in `tracing.ts` (row above), matched by `isGameEventsStreamPath(url)` — a pure, unit-tested predicate (`apps/shuffler/test/game-events-route-filter.test.ts`, same style as `telemetry-sampler.ts`'s tested pure functions) exported from its own file so the match logic is tested independently of the instrumentation wiring. Open/close are logged instead: `log.info("game-events: browser tab opened"/"closed", { "game.game_id": ... })`. **Confirmed by observation in a real run**: because the route's incoming-request span is suppressed, these logs have no active span and land as standalone rows with no trace/span id — expected, not a bug, and the correct trade for not wanting a tab-lifetime span. This is the fleet's **first use of `ignoreIncomingRequestHook`** — see the watch point below for reusing this shape on a future long-lived-connection route. |
...
started only when the event is actually applied). Until this ticket **the Shuffler created zero
manual spans** — it lived entirely off auto-instrumentation plus stamping attributes onto
whatever span already exists, which is why `markCurrentSpanAsError`/`setCommonSpanAttributes`
matter so much there. `cardReturnedDispatch.ts` follows the Tabletop's `spineEventDispatch.ts`
shape exactly (same file's row below): `propagation.extract(ROOT_CONTEXT, { traceparent })` from
the received envelope, `context.with(parentContext, () => tracer.startActiveSpan(...))`, fallback
to `ROOT_CONTEXT` on a missing/malformed `traceparent`, both spans wrapped in `try`/`finally` so
`span.end()` fires on every exit path. **Deliberately no `span.recordException`** anywhere in it
(it's `addEvent` under the hood, forbidden by Invariant 2) — failures go through
`markCurrentSpanAsError` (attributes) + `log.error` (stack trace) instead, the house pattern
below. `card_return.outcome` is stamped on every branch: `applied`, `duplicate`, `invalid`, the
...
### The Tabletop verifies a send-then-commit flow with a fake Spine on port 4600

`apps/tabletop/verify.sh`'s server has no real Spine to talk to (only the Shuffler's verify suite
spawns one — see the row above), but the library-portal swallow (ticket 12,
`sendCardReturned.ts`/`cardReturned.ts`) is a genuine send-then-commit: the browser awaits a real
2xx from `POST /api/tables/:tableName/cards/return` before it deletes anything, so verification
needs *something* answering on `SPINE_URL`'s default port or the flow can never complete.
`apps/tabletop/test/verification/support/fakeSpine.ts` is a minimal `http.createServer` that
accepts every `POST /tables/:tableId/events` with 201 — wired in as `globalSetup`/`globalTeardown`
(`apps/tabletop/test/verification/support/globalSetup.ts`, referenced from
`playwright.config.ts`) so it's up for the whole Playwright run rather than per-spec. It answers
"the Spine confirmed" for any send-then-commit spec; a spec that needs to inspect what was actually
sent still wants a purpose-built fake (as `sendCardReturned.test.ts`'s unit-level
`createFakeSpineServer` already does). **Worth reusing, not reinventing**: any future
send-then-commit flow added to the Tabletop's verify suite should extend/reuse this fake rather
than adding a second stand-in Spine.
...
### A runtime file that reads `contracts/` by relative path is a Docker-flattening trap

Any file that reads from `contracts/` at **runtime** (not build time) — currently
`incomingEventValidation.ts` (Shuffler) and `contractValidation.ts` (Tabletop), plus the Spine's
`event_contract.rb` — needs its container path handled deliberately, because the two Node ships'
Dockerfiles disagree on shape: **the Tabletop's preserves `/repo/apps/tabletop` nesting; the
Shuffler's flattens the whole workspace to `/app`.** A `path.join(process.cwd(), "..", "..",
"contracts")`-style default that resolves correctly in dev (and in jest, and via `./run`) silently
stops resolving once flattened — the build succeeds, the container boots clean, and only the
first real request that needs the schema fails. `verify-container-boot.sh` does **not** catch this
(see the fleet `CLAUDE.md`'s container-cost note) — a boot check that doesn't exercise the route
looks green regardless. The fix, now used by both `incomingEventValidation.ts` and
`event_contract.rb`: a `CONTRACTS_DIR` env var override, with the Dockerfile `COPY contracts
./contracts` and `ENV CONTRACTS_DIR=/app/contracts` for whichever image is flattened. If you add a
**third** file that reads `contracts/` at runtime, check which Dockerfile it ships in before
trusting a relative default — don't assume "it worked locally" generalizes to the built image.
