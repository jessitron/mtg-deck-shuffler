...
- **2026-08-11, ticket 02 of `spine-roda-rewrite`: the Spine got OTel wiring back, from its first
  ...
  - **Querying the wrong MCP server looks identical to a wiring bug.** The default `honeycomb` MCP
    ...
    (renamed to `mtg-spine` the same day, see below).
  ...
  - **Same-day rename**: `OTEL_SERVICE_NAME` (and `config/telemetry.rb`'s fallback default)
    changed from `"spine"` to `"mtg-spine"`, to match the fleet's `mtg-tabletop`/`mtg-deck-shuffler`
    naming convention. Re-verified live — a fresh trace landed under the new `mtg-spine` dataset.
