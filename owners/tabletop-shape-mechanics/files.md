...
- `apps/tabletop/src/server/spineEventDispatch.ts` — `dispatchSpineEvent(tableName, event)`, the
  ...
  `applyCardArrival`. Not this owner's mechanics territory (no ShapeUtil, no selection state) but
  recorded here since it's `card.played`'s only production entry point.
- `apps/tabletop/src/server/testSeedRoute.ts` — `handleTestCardSeed`, a test-only HTTP seam at
  `POST /test/tables/:tableName/cards`, mounted
