...
| `test/port-tabletop/gateways.test.ts`, `sendToTable.test.ts` | Gateways record/fail; send-then-commit sending half. `gateways.test.ts` gained a `"buildSeatJoinedEvent commanders"` describe block (table-layout ticket 18, 2026-08-09): 0/1/2 commanders, `backImageUrl` derived from `twoFaced`, no `face` on the entry — same convention as `cardPlayedEvent.test.ts` |
