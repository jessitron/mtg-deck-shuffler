...
| `src/port-spine/sendToSpine.ts` | `joinSpineBestEffort()` calls `buildSeatJoinedPayload()` and POSTs the result (plus identity) to the Spine's `/join` as a `SpineJoinRequest` |
