...
it is the de-facto spec of F0) and, on the Tabletop side, the real ajv validation in
`apps/tabletop/src/server/contractValidation.ts` that replaced `cardArrival.ts`'s
hand-rolled `validationError` (which, before it was deleted, briefly required
`frontImageUrl: string` + `backImageUrl: string | null` + `owner`/`isCommander` all
at once — table-layout ticket 18 landed the same day as this one).
