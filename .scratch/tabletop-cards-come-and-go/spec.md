...
- **Who deletes the table on restart** — decided to be something *outside* the Tabletop,
  but which component and triggered how is not yet specifiable (touches map 6 and
  possibly the Spine). Restart's lingering-cards behavior, documented in ticket 03,
  stays as-is for now.
